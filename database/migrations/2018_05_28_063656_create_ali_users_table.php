...
class CreateAliUsersTable extends Migration
{
	...
	public function up()
	{
		Schema::create('ali_users', function(Blueprint $table) {
            ...
            $table->char('user_status')->default('T')->comment('用户状态（Q/T/B/W）。 Q代表快速注册用户 T代表已认证用户 
            B代表被冻结账户 W代表已注册，未激活的账户');
            $table->char('is_certified')->default('T')->comment('是否通过实名认证。T是通过 F是没有实名认证。');
            $table->char('gender')->default('M')->comment('【注意】只有is_certified为T的时候才有意义，否则不保证准确性.
             性别（F：女性；M：男性）。');
            $table->timestamps();
		});
	}
