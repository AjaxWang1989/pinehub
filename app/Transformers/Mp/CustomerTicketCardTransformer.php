...
class CustomerTicketCardTransformer extends TransformerAbstract
{
    public function transform(CustomerTicketCard $model){
        return [
            ...
            'begin_at'  => isset($model->beginAt) ? $model->beginAt->format('Y-m-d') : null,
            'end_at'    => isset($model->endAt) ? $model->endAt->format('Y-m-d') : null,
            'status'    => $model->status,
            ...
            'least_cost' => isset($model->card->card_info['least_cost']) ? round($model->card->card_info['least_cost'],2) : null,
            'reduce_cost' => isset($model->card->card_info['reduce_cost']) ? round($model->card->card_info['reduce_cost'],2) : null,
...
//            'record_count' => $model->card->recordCount,
            'begin_timestamp' => isset($model->card->card_info['base_info']['date_info']['begin_timestamp']) ? date('Y-m-d',$model->card->card_info['base_info']['date_info']['begin_timestamp']) : '',
            'end_timestamp' => isset($model->card->card_info['base_info']['date_info']['end_timestamp']) ? date('Y-m-d',$model->card->card_info['base_info']['date_info']['end_timestamp']) : '',
