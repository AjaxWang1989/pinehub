...
class Shop extends Model implements Transformable
{
    ...
    public static function boot()
    {
        ...
        self::creating(function (Shop $shop) {
            $appManager = app(AppManager::class);
            if(!$shop->code) {
                $shop->code = app('uid.generator')->getUid(SHOP_CODE_FORMAT, SHOP_CODE_SEGMENT_MAX_LENGTH);
            }
            $shop->appId = $appManager->currentApp->id;
            $shop->wechatAppId = $appManager->officialAccount ? $appManager->officialAccount->appId : null;
            $shop->aliAppId = $appManager->aliPayOpenPlatform->config['app_id'];
            return $shop;
        });
    }
