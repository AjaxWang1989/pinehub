...
class WechatOpenPlatformRoutes extends WebRoutes
{
    protected function subRoutes($router)
    {
        tap($router, function (Router $router) {
            $router->get('/{appId}/auth', ['as' => 'open-platform.auth.callback', 'uses' => 'OpenPlatformController@componentAuthCallback']);
            $router->get('/auth', ['as' => 'open-platform.auth', 'uses' => 'OpenPlatformController@componentAuth']);
